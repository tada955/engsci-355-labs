...
Run Jaamsim by opening your VSCode project and the clicking the run button and select GUIFrame.
...
In your VSCode project you should have a folder called **sim\_custom** under the Explorer tab on the left-hand side in VSCode.
First right-click on this folder and select New Java Package.
Enter **labs** for the name of the package and press Enter.

::: {#fig-create_package_1}
![](figures/vscode_package_creation_1.png)

First Step in Creating a New Package

:::

::: {#fig-create_package_2 .column-page-right}
![](figures/vscode_package_creation_2.png)

Second Step in Creating a New Package

:::

A new folder called **labs** should have been created within the **sim\_custom** folder.
Right click on the newly created **labs** folder and select New Java File $\rightarrow$ Class.
Name the Class **RadiologyControlUnit** and press Enter.

::: {#fig-create_class_1}
![](figures/vscode_class_creation_1.png)

First Step in Creating a New Class

:::

::: {#fig-create_class_2 .column-page-right}
![](figures/vscode_class_creation_2.png)
...
If you want to copy and paste this make sure the quotes are copied correctly and the returns (arrows) are removed.
Alternatively there is a new sim\_custom.inc file here XXX on Canvas under Jaamsim Lab 1 that you can use directly.
...
Once you have updated the sim\_custom.inc file, restart Jaamsim.
...
First add the following imports under the package declaration.
**Note** These code snippets for this lab are provided in a separate file for you here XXX.
...
We can use the SampleAverage output of the TimeInSystem object in the Simulation's RunOutputList to output the mean time in system for each replication. **Note** The SampleAverage is divided by 1[h] to give a raw number in hours for later processing in Python. Otherwise JaamSim writes an h to the data file.
...
First download the Python analysis file provided, then change name of the .dat and .log files to match yours and make sure they are in the same directory as the Python file, then run the Python file.
...
You will need to add an entry to the **RunOutputList**.
